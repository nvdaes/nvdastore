...
const Home = () => {
  ...
  const handleSubmit = (event) => {
    event.preventDefault();
		    setUrl(`https://addonstore.nvaccess.org/${lang}/${channel}/${apiVer}.json`);
  }
  ...
  return (
	...
	<p>Addons available on the <a href="https://github.com/nvaccess/addon-datastore"target="_blank" rel="noopener noreferrer">NV Access add-on datastore repository</a> (external)</p>
	  ...
	  <main>
        ...
        {data.sort(function(a, b){return b.displayName - a.displayName}) &&
        data.map((item, index) => {
          return (
		  <>
      ...
      <li>Minimum NVDA version: {item.minNVDAVersion.major}.{item.minNVDAVersion.minor}.{item.minNVDAVersion.patch}</li>
      <li>Last tested NVDA version: {item.lastTestedVersion.major}.{item.lastTestedVersion.minor}.{item.lastTestedVersion.patch}</li>
